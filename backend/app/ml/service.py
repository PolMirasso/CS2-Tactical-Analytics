...
def train_model(session: Session, user: User) -> SitePredictor:
    global _predictor, _loaded
    feats, targets, meta = build_dataset(session, user)
    predictor = SitePredictor.train(feats, targets, meta)
    _persist(predictor, feats, targets)
    with _lock:
...
def _persist(predictor: SitePredictor, feats: list[dict], targets: list[str]) -> None:
    import joblib
    ...
    try:
        _export_snapshot(feats, targets)
    except Exception:
        pass


def _export_snapshot(feats: list[dict], targets: list[str]) -> None:
    """Reproducibility: dump the training table to dataset_dir (best effort)."""
    if not feats:
        return
    import pandas as pd
